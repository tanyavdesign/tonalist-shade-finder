...
1. **Exposure runs on the camera's own auto pipeline; white balance is locked to manual when the camera allows it.** An earlier version tried `applyConstraints` with BOTH `whiteBalanceMode: manual` and `exposureMode: manual` at once, to stop auto white balance from silently correcting the exact thing being measured. On phone cameras this destabilised the feed: the picture would darken dramatically mid-session, recovering only when the app was backgrounded and foregrounded (the browser tearing down and re-acquiring the camera, dropping the forced override). Both locks were dropped together at the time, and the camera's actual mode was just reported honestly in diagnostics instead.

   Real reading screenshots later showed the actual failure was auto white balance, not exposure: under a full-screen white/grey flash, a small warm (skin-toned) patch inside an otherwise near-neutral frame gets pushed pink/magenta — classic grey-world AWB, averaging the whole scene toward neutral and "correcting" the one region that was never wrong. Worse, this breaks `calibrationGains()`'s core assumption that the camera's colour response is identical across every calibration flash in one reading — AWB re-deciding per flash violates that outright, which matters more than any single flash's colour being imperfect. So `lockCamera()` now attempts `whiteBalanceMode: manual` ALONE (never touching `exposureMode`, which is the half already proven to cause the darkening bug) and reports success/failure honestly. Locking to an imperfect white-balance value is fine — the calibration math only needs a *stable* distortion to correct for, not a *zero* one. A 1px canvas blur is the only frame processing applied, purely to knock down sensor noise before averaging — no sharpening, no tone/colour mapping.
2. **Seven full-screen flashes**, sampled from the live video: `ambient` (black), white, grey, red, green, blue, `warm` (warm-tinted near-white, `#FFE8B8`). `warm` was originally `#FFC832`, a much dimmer, more saturated amber — once it became the actual tone-reading flash (see below), that dimness (only ~78% max luma, and just 50/255 on blue) was starving the reading of light on top of the existing "screen is weak at a distance" problem, so it was brightened toward full luminance while staying visibly warm-tinted. **Two disjoint roles, and this distinction is the core design principle of the whole pipeline: black/white/grey/red/green/blue are calibration-only — known colours whose entire job is characterising how this specific camera+screen pair distorts a known colour into whatever the sensor reports. `warm` is the one and only flash the actual skin-tone reading is taken under, run through the correction the calibration flashes derive. Calibration flashes are never read for tone; `warm` is never used to derive the correction.** An eighth flash, `skinref` (`#C68642`), used to be sampled every reading purely as an extra diagnostic data point and was never read by anything — removed outright, and that time went into holding every remaining flash longer instead (900ms–1000ms, up from as low as 750ms), since a longer hold is what auto white balance actually needs to settle before being sampled.
...
5. **Calibration gains**, `calibrationGains()`, fit ONE per-channel correction using all five calibration flashes (white, grey, red, green, blue) at once: for each channel, a least-squares scale fit through the origin across the (observed, target) pairs from all five — `gain[c] = Σ(target·observed) / Σ(observed²)` — normalised to a geometric mean of 1, THEN clamped to [0.5, 2.0]. That order matters and was wrong for a while: clamping each channel's raw ratio to [0.5, 2.0] *before* normalising meant that whenever the screen's light was dim relative to what a full-brightness calibration flash needed (routine, given how weak the screen is as a light source at any real distance), the raw per-channel ratios could all individually exceed 2.0 by very different amounts — real diagnostics once showed 5.9 / 6.4 / 2.5 — and clamping first made all three saturate at exactly 2.0, so normalising three identical numbers against their own geometric mean always returned `[1,1,1]`: a silent no-op that discarded a real, substantial, per-channel-different correction and left the raw (often pink/magenta) camera output in its place. This was verified against a real capture's diagnostics: the old order reproduced that reading's actual (wrong, dusty-mauve) output exactly, and normalising first before the same clamp turned it into a plausible warm tan from the identical raw data. This replaced two earlier, narrower attempts: one used only the pure R/G/B flashes (a single noisy data point per channel), the other added a second pass off the grey flash alone; neither incorporated all the available reference colours at once, and both left a systematic blue-grey cast in the readings. This is still a diagonal ("von Kries") correction, the same family real camera white-balance algorithms use — it can correct per-channel scale, not full 3×3 cross-channel colour rotation, which was judged not worth the complexity given the shade library is already placeholder data.
...
- **Exposure runs unlocked on every device, by design.** White balance is locked to manual when the camera's capabilities offer it (see the pipeline section above); check `meta.lock` in diagnostics to see whether a given reading actually got the lock or fell back to fully auto. Either way, the channel-gain and ambient-subtraction math is what corrects the camera's colour response in software; readings taken under inconsistent or changing lighting mid-sequence, or on a device that refuses the white-balance lock, are the ones most likely to be wrong.
