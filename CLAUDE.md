...
5a. **Reference gains**, from the `grey` flash (`#808080`) — added to catch the screen's own colour cast, since channel gains only correct camera sensor sensitivity, not what the screen itself actually emits (phone OLEDs commonly skew cool/blue at max brightness). `referenceGains()` compares the ambient-subtracted, channel-gain-corrected grey response against grey's known linear value (equal in all three channels) and derives a second diagonal correction the same way (geometric-mean-normalised, clamped to [0.5, 2.0]) — a genuinely neutral flash makes any per-channel imbalance in the response the cast itself. This first went through `skinref` (`#C68642`) instead, which sounded like the more on-the-nose reference target, but its low blue content (R198 G134 B66) reflecting off skin routinely produced a blue signal too faint to trust, so it fell back to a no-op every time — that's why readings kept coming out blue-grey. The `skinref` flash is still sampled and shown in diagnostics but no longer used for correction.
...
- A small (150px) self-view (`#miniPreview`) is shown during the flash sequence only, above the flash overlay, so the user can see their framing while the full-screen flash would otherwise hide them completely. It shares the same `MediaStream` as the main video element and plays no part in sampling — kept intentionally small so it doesn't meaningfully change the light reaching the face. It also overlays the same alignment oval and sample-zone boxes as the main guide (`drawGuideOn()`, shared by both), so it shows the actual region being analysed, not just a plain face crop.
